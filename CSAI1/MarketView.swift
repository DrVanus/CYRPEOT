...
struct MarketView: View {
        ...
        NavigationView {
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    columnHeader
                    if vm.filteredCoins.isEmpty && vm.isMarketLoading {
                        ...
                        if let error = vm.coinError {
                            DataUnavailableView(message: error) {
                                Task { await vm.fetchMarketDataMulti() }
                            }
                        } else {
    ...
    // MARK: - Segment Row Helpers
    /// A single segment button, extracted to simplify the view builder.
    private func segmentButton(_ seg: MarketSegment) -> some View {
        Button {
    ...
    private var segmentSearchButton: some View {
        Button {
    ...
    /// A skeletal placeholder for loading rows.
