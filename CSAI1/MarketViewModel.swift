...
// MARK: - Fallback model for CoinPaprika API
struct CoinPaprikaTicker: Decodable {
    let id: String
    let symbol: String
    let name: String
...
class MarketViewModel: ObservableObject {
  ...
  init() {
    ...
    Task { @MainActor in
      async let markets = fetchMarketDataMulti()
      async let global = fetchGlobalMarketDataMulti()
      await markets
      await global
      applyAllFiltersAndSort()
    }

    startAutoRefresh()                 // continue background updates
  }
  ...
  func fetchGlobalMarketDataMulti() async {
      ...
      do {
          ...
          // Fallback so summary view can show something instead of loading forever
          let fallback = GlobalMarketData(
            totalMarketCap: ["usd": globalMarketCap],
            totalVolume: ["usd": volume24h],
            marketCapPercentage: ["btc": btcDominance, "eth": ethDominance]
          )
  ...
  func fetchMarketDataMulti() async {
      ...
      do {
          ...
          do {
              guard let fallbackURL = URL(string: "https://api.coinpaprika.com/v1/tickers") else { throw error }
              let (fallbackData, _) = try await session.data(from: fallbackURL)
              let paprikaList = try JSONDecoder().decode([CoinPaprikaTicker].self, from: fallbackData)
              let mapped = paprikaList.prefix(100).compactMap { item -> MarketCoin? in
                  guard let quote = item.quotes["USD"] else { return nil }
                  return MarketCoin(
                      id: UUID(), // or UUID(uuidString: item.id) ?? UUID()
                      symbol: item.symbol,
                      name: item.name,
                      price: quote.price,
                      dailyChange: quote.percent_change_24h ?? 0,
                      hourlyChange: 0,
              ...
              await MainActor.run {
                  self.coins = Array(mapped)
                  self.loadFavorites()
                  self.applyAllFiltersAndSort()
              }
              return
          } catch {
              print("Fallback CoinPaprika fetch failed:", error)
          }
      }
  }
