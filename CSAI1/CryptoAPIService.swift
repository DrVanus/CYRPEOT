...
class CryptoAPIService {
    ...
    func fetchCoinGeckoMarkets(
        ...
        // Debug: print the final URL to the console
        print("🔗 Fetching CoinGecko markets from URL: \(url.absoluteString)")
        ...
        if let httpResponse = response as? HTTPURLResponse, !(200...299).contains(httpResponse.statusCode) {
            print("🚨 CoinGecko HTTP error: \(httpResponse.statusCode)")
            throw CryptoAPIError.requestFailed
        }

        // Decode JSON into MarketCoin array
        do {
            let coins = try JSONDecoder().decode([MarketCoin].self, from: data)
            return coins
        } catch {
            print("❌ Decoding error fetching CoinGecko markets: \(error)")
            throw CryptoAPIError.decodingError
        }
    }
